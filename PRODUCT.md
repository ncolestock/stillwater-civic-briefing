...
A hyper-local newspaper and two audio episodes a week. Monday is the week's current events. Wednesday is a biography of one person who holds an office, and it does not air when the record cannot tell that life. Wednesday names the title once and does not describe the office. The official name is New Saint Croix Union. In body copy the short name may be NSCU or the Union. There is no named host.
...
GitHub Actions then runs `astro build` and deploys Pages. Monday at 4:00 p.m. an automation writes the week's current events from published stories. Wednesday at 4:00 p.m. another writes one biography, under the test in `STANDARDS.md`, and commits nothing when the test fails. The Action calls the xAI Text to Speech API (`podcast.json` field `tts_voice_id`, default `eve`) when the repository secret `XAI_API_KEY` is set.
